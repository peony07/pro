...
public class baidu_meinv {
	...
	public static void main(String[] args) throws Exception {
		baidu_meinv aa = new baidu_meinv();
		...
		for(int i=0;i<list.size();i++){
			...
			String tag3Url=url+"data/imgs?&sort=0&tag3=&p=channel&from=1";
			tag3Url+="&col="+map.get("col").toString()+"&tag="+map.get("tag2").toString();
			long totalNum=aa.getTotalNum(tag3Url+"&pn=0&rn=1");
			log.info(map.get("col_name").toString()+">"+map.get("tag2_name").toString()+"("+totalNum+")"+":"+tag3Url);
			if("全部".equals(map.get("tag2_name").toString())){
				continue ;
			}
			int startIndex=0;
			// 开始抓取图片
			...
			for (int j = startPage; j <= totalNum/rn; j++) {
				startIndex=j*rn;
				aa.forum(tag3Url + "&pn="+startIndex+"&rn="+rn,map.get("tag2_name").toString().trim());
			}
		}
		log.info("----图片抓取完毕----");
	}
	...
	public List getCategory(String categoryUrl) throws Exception {
		Connection conn_col = Jsoup.connect(categoryUrl).timeout(TIME_OUT);
		...
		for(int p=0;p<alinks_col_list.size();p++){
			...
			for (int i = 0; i < alinks_tag2.size(); i++) {
				...
				map_tag2.put("tag2_name", tag2_name);
				
				Map map_result=new HashMap();
				map_result.putAll(col_map);
				map_result.put("tag2", map_tag2.get("tag").toString());
	...
	public void forum(String targetURL,String tag2Name) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void post(String name,String tag2Name,String picURL,String picName) throws Exception {
		if (picURL.indexOf("http://") == -1) {
			picURL = "http://" + dirString + "/" + picURL;
		}
		ThreadPictures threadPictures = new ThreadPictures();
		threadPictures.setPIC_DIR(PIC_DIR+"/"+CommonUtil.getNewDirName(name)+"/"+CommonUtil.getNewDirName(tag2Name));
